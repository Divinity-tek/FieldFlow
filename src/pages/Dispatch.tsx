...
import {
  ...
  LayoutGrid, List, GripVertical, Wrench, FolderOpen, Hash,
} from "lucide-react";
...
type TicketExtras = { client_ticket_number?: string; project_id?: string; tools?: string[] };
...
const DispatchTickets = () => {
  ...
  const estimatedTotal =
    (Number(form.estimated_hours) * Number(form.hourly_rate))
  ...
  const stats = {
    ...
    active: tickets.filter((t: any) => ["dispatched", "in_progress"].includes(t.status)).length,
  ...
  return (
    <AppLayout title="Dispatch Tickets" subtitle="Hands & Feet Support — SOW Management">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <Card><CardContent className="pt-4 pb-3">
          <div className="flex items-center justify-between">
            <div><p className="text-xs text-muted-foreground">Est. Revenue</p><p className="text-2xl font-bold">${stats.totalEstimated.toLocaleString()}</p></div>
            ...
            <div>
              ...
              <Select value={engineerFilter} onValueChange={setEngineerFilter}>
                ...
                <SelectContent>
                  ...
                  {engineers.map((e: any) => (
                    <SelectItem key={e.id} value={e.id}>{e.profiles?.full_name ?? e.specialty}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
      ...
      {canManage && selectedIds.length > 0 && (
        <div className="flex flex-wrap items-center gap-2 mb-3 p-2 rounded-md border bg-muted/40">
          ...
          <Select onValueChange={(v) => bulkUpdateMutation.mutate({ ids: selectedIds, updates: { engineer_id: v === "unassigned" ? null : v } })}>
            <SelectTrigger className="h-8 w-[180px]"><SelectValue placeholder="Assign engineer…" /></SelectTrigger>
            <SelectContent>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                <div>
                  ...
                  <Input type="number" min="0" step="0.5" value={form.estimated_hours} onChange={(e) => setForm({ ...form, estimated_hours: Number(e.target.value) })} />
                </div>
                <div>
              ...
              <div className="bg-muted/50 rounded-lg p-4 border">
                <div className="space-y-1.5 text-sm">
                  <div className="flex justify-between"><span className="text-muted-foreground">Labor</span><span>{form.estimated_hours}h × ${form.hourly_rate} = ${(form.estimated_hours * form.hourly_rate).toFixed(2)}</span></div>
                  ...
                  <TabsContent value="charges" className="mt-4">
                    <div className="bg-muted/30 rounded-lg border p-4 space-y-3">
                      ...
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between"><span className="text-muted-foreground">Labor ({viewTicket.estimated_hours}h × ${Number(viewTicket.hourly_rate).toFixed(2)})</span><span>${(viewTicket.estimated_hours * viewTicket.hourly_rate).toFixed(2)}</span></div>
                        ...
                        <Separator />
                        <div className="flex justify-between font-bold text-base"><span>Estimated Total</span><span className="text-primary">${Number(viewTicket.estimated_charges ?? 0).toFixed(2)}</span></div>
                      </div>
                ...
                {!isEngineer && (
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(v) => setEditForm({ ...editForm, engineer_id: v === "unassigned" ? "" : v })}
                      disabled={engineersLoading || !!engineersError}
                    >
                      <SelectTrigger aria-invalid={!!engineersError} className={engineersError ? "border-destructive" : ""}>
                        <SelectValue placeholder={engineersLoading ? "Loading engineers…" : "Select engineer"} />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="unassigned">Unassigned</SelectItem>
                        {engineers.map((e: any) => {
